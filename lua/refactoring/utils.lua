...
function M.node_contains(a, b)
  ...
  if end_col == a_end_col then end_col = end_col - 1 end

  return ts.is_in_node_range(a, start_row, start_col) and ts.is_in_node_range(a, end_row, end_col)
end
...
function M.nodes_to_text_set(bufnr, nodes)
  ...
  for _, node in pairs(nodes) do
    ...
    if text ~= nil then out[text] = true end
  end
  return out
end
...
function M.region_above_node(node)
  ...
  local node_for_region = assert(node:named_descendant_for_range(scope_region:to_ts()))
  ...
  for _, import_node in ipairs(import_nodes) do
    if node_for_region:type() == import_node then
      scope_region = M.region_above_node(assert(node_for_region:next_named_sibling()))
    end
  end
...
local function is_comment_or_decorator_node(node)
  ...
  for _, node_type in ipairs(comment_and_decorator_node_types) do
    if node_type == node:type() then return true end
  end
...
function M.get_first_node_in_row(node)
  ...
  while true do
    ...
    if parent_row ~= start_row then break end
    first = parent
  end
...
function M.get_non_comment_region_above_node(refactor)
  local prev_sibling = M.get_first_node_in_row(refactor.scope):prev_named_sibling()
...
function M.get_selected_locals(refactor)
  ...
  local local_defs =
    iter(refactor.ts:get_local_defs(refactor.scope, refactor.region)):map(node_to_parent_if_needed):totable()
  local region_refs =
    iter(refactor.ts:get_region_refs(refactor.scope, refactor.region)):map(node_to_parent_if_needed):totable()
