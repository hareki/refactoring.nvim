...
function M.get_debug_points(refactor, opts)
  ...
  local current_line = api.nvim_buf_get_lines(refactor.bufnr, cursor.row - 1, cursor.row, true)[1]
  ...
  local current = language_tree:named_node_for_range(range, { ignore_injections = false })
  assert(current)
  ...
  while current and not is_statement do
    ...
    if not is_statement then current = current:parent() end
  end
  ...
  if current and not is_indent_scope then
    ...
    if current and is_indent_scope then
      ...
      local below_line = api.nvim_buf_get_lines(refactor.bufnr, start_row + 1, start_row + 2, true)[1]
      _, non_white_space = below_line:find "^%s*()" --[[@as integer, integer]]

      current = current:named_descendant_for_range(start_row + 1, non_white_space, start_row + 1, non_white_space + 1)
        or current
    end
  end
