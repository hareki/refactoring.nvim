...
function M.get_printf_statement(opts, refactor)
  ...
  if custom_printf_statements then
    if #custom_printf_statements > 1 then
      printf_statement = assert(ui.select(custom_printf_statements, "printf: Select a statement to insert:")) --[[@as string]]
    else
...
local function text_edit_insert_text(refactor, opts, printf_statement, content, point)
  ...
  if start_row == end_row and start_col == end_col then start_row = start_row - 1 end
  ...
  local start_comment = refactor.code.comment "__AUTO_GENERATED_PRINTF_START__"
  ...
  table.insert(refactor.text_edits, text_edits_utils.insert_new_line_text(range, text, opts))
end
...
---@param i integer
local function text_edits_modify_count(refactor, debug_path, escaped_printf_statement, lines, row_num, i)
  local escaped_debug_path = vim.pesc(debug_path)
  local count_pattern = debug_path ~= "" and escaped_debug_path .. " " .. "(%d+)" or "(%d+)"
  local before_count_pattern = debug_path ~= "" and escaped_debug_path .. " " .. "()%d+" or "()%d+"
  local after_count_pattern = debug_path ~= "" and escaped_debug_path .. " " .. "%d+()" or "%d+()"
  ...
  if current_count ~= text then
    local range = Region:from_values(refactor.bufnr, row_num, _start, row_num, _end - 1)
    table.insert(refactor.text_edits, text_edits_utils.replace_text(range, text))
  end
end
  ...
  Pipeline:from_task(
    ...
    :add_task(
      ...
      function(refactor)
        ...
        local insert_pos, path_pos = debug_utils.get_debug_points(refactor, opts)
        ...
        local escaped_printf_statement =
          printf_statement:gsub("%%%%", "%%%%%1"):gsub("([%^%$%(%)%[%]%*%+%-%?])", "%%%%%1")
        local text_to_count_pattern = debug_path ~= "" and ("%s %%d+"):format(vim.pesc(debug_path)) or "%d+"
        local text_to_count = refactor.code.print {
          statement = escaped_printf_statement,
          content = text_to_count_pattern,
        }

        local lines = api.nvim_buf_get_lines(refactor.bufnr, 0, -1, false)

        ---@type integer[]
        local current_lines_with_text = {}
        for row_num, line in ipairs(lines) do
          if (line):find(text_to_count) ~= nil then table.insert(current_lines_with_text, row_num) end
        end
        local should_replace = vim.tbl_contains(current_lines_with_text, cursor.row) and opts.below
        ...
        for i, row_num in ipairs(current_lines_with_text) do
          ...
          if row_num == cursor.row and not should_replace then
            should_replace = true
            local ok2, error = pcall(text_edit_insert_text, refactor, opts, printf_statement, content, insert_pos)
            if not ok2 then return ok2, error end
          else
            if row_num == cursor.row then should_replace = false end
            text_edits_modify_count(refactor, debug_path, escaped_printf_statement, lines, row_num, i)
          end
        end
