...
function M.not_ready()
  return false, "Sorry this refactor is not ready yet!  But I appreciate you very much :)"
end
...
local function preview_highlight(bufnr, ns, edit_set)
  ...
  for _, edit in pairs(edit_set) do
    if edit.newText == "" then
      api.nvim_buf_set_extmark(bufnr, ns, edit.range.start.line, edit.range.start.character, {
...
function M.refactor_apply_text_edits(refactor)
  if not refactor.text_edits then return true, refactor end
  ...
  for _, edit in pairs(refactor.text_edits) do
    ...
    if not edits[bufnr] then edits[bufnr] = {} end
  ...
  for bufnr, edit_set in pairs(edits) do
    if ns then preview_highlight(bufnr, ns, edit_set) end
...
function M.create_file_from_input(refactor)
  ...
  if not file_name or file_name == "" then return false, "Error: Must provide a file name" end
...
function M.ensure_code_gen(refactor, code_gen_operations)
  for _, code_gen_operation in ipairs(code_gen_operations) do
    if refactor.code[code_gen_operation] == nil then
      return false, ("No %s function for code generator for %s"):format(code_gen_operation, refactor.lang)
    end
  end
  return true, refactor
end
...
function M.operator_setup(refactor)
  ...
  if not ok then return ok, scope end

  refactor.region = region
  refactor.region_node = region_node
  refactor.scope = scope

  if refactor.scope == nil then return false, "Scope is nil" end
...
local function success_message(refactor)
  ...
  if refactor.success_message and config.show_success_message then
    vim.notify(refactor.success_message, vim.log.levels.INFO, { title = "refactoring.nvim" })
  end
  return true, refactor
end

function M.post_refactor()
  return Pipeline:from_task(M.refactor_apply_text_edits):add_task(success_message)
end
